...
class LogParser:
    ...
    def _parse_clients(self, log):
        ...
        tmp = search(r'\[([^\s\]]+Z) .* Start ', log).group(1)
        start = self._to_posix(tmp)

        misses = len(findall(r'rate too high', log))

        tmp = findall(r'\[([^\s\]]+Z) .* sample transaction (\d+)', log)
    ...
    def _parse_primaries(self, log):
        ...
        tmp = findall(r'\[([^\s\]]+Z) .* Created B\d+\([^ ]+\) -> ([^ ]+=)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        proposals = self._merge_results([tmp])

        tmp = findall(r'\[([^\s\]]+Z) .* Committed B\d+\([^ ]+\) -> ([^ ]+=)', log)
        tmp = [(d, self._to_posix(t)) for t, d in tmp]
        ...
        boot_line = search(r'\[([^\s\]]+Z) .* booted on (\d+.\d+.\d+.\d+)', log)
